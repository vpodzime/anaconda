...
class PartTypeSpoke(NormalTUISpoke):
    ...
    def _ensure_init_storage(self):
        ...
        if self._orig_clearpart_type is not None or (self._orig_mount_assign and not self._do_mount_assign):
...
class MountDataRec(kickstart.MountData):
    """ An artificial subclass also recording changes """
    def __init__(self, *args, **kwargs):
        super(MountDataRec, self).__init__(*args, **kwargs)
        self.modified = False
        self.orig_format = None

class MountPointAssignSpoke(NormalTUISpoke):
    """ Assing mount points to block devices """
    category = SystemCategory

    def __init__(self, data, storage, payload, instclass):
        NormalTUISpoke.__init__(self, data, storage, payload, instclass)
        self.title = N_("Assing mount points")
        self._container = None
        self._mds = None
        ...
        for device in filter(self._is_dev_usable, self.storage.devicetree.leaves):
            ...
            for ks_md in self.data.mount.dataList():
                if device is self.storage.devicetree.resolve_device(ks_md.device):
                    ...
                    # let's just copy it
                    mdrec = MountDataRec(device=ks_md.device, mount_point=ks_md.mount_point,
                ...
                mdrec = MountDataRec(device=device.path, mount_point=mpoint, format=fmt, reformat=False)
    ...
    def apply(self):
        """ Apply our selections. """
        for md in self._mds.values():
            if md.modified and (md.reformat or md.mount_point):
                self.data.mount.add_mount_data(md)
...
class ConfigureDeviceSpoke(NormalTUISpoke):
    ...
    def refresh(self, args=None):
        ...
        fmt = get_format(self._md.format)
        if fmt and fmt.mountable:
            self._container.add(TextWidget("Mount point: %s" % self._md.mount_point or "none"), self._assign_mount_point, self._md)
        else:
            self._container.add(TextWidget("Mount point: [%s]" % fmt.name), lambda x: self.redraw(), None)
        self._container.add(TextWidget("Format: %s" % self._md.format or "none"), self._set_format, self._md)
        if ((self._md.orig_format and self._md.orig_format != self._md.format) or self._md.mount_point == "/"):
            # changing format implies reformat and so does "/" mount point
            self._container.add(CheckboxWidget(title="Reformat", completed=self._md.reformat), lambda x: self.redraw(), None)
        else:
            self._container.add(CheckboxWidget(title="Reformat", completed=self._md.reformat), self._switch_reformat, self._md)

        self.window.add_with_separator(self._container)

        message = _("Choose from above to assign mount point and/or set format.")
        self.window.add_with_separator(TextWidget(message))
...
class SetFormatDialog(EditTUIDialog):
    ...
    def prompt(self, args=None):
        if self.value is None:  # first run or nothing entered
            return Prompt(_("New format for the device%s") % (" [%s]" % self._md.format if self._md.format else ""))
        else:
            self.apply()
            self.close()
    ...
    def input(self, args, key):
        if not key:
            ...
            if key in self._supported_filesystems:
                ...
                print(_("Supported filesystems: %s") % ", ".join(self._supported_filesystems))
                return InputState.DISCARDED

    def apply(self):
        if self.value != self._md.format:
            self._md.reformat = True
            self._md.format = self.value
...
class SetMountPointDialog(EditTUIDialog):
    ...
    def apply(self):
        self._md.mount_point = self.value
        if self._md.mount_point == "/":
            self._md.reformat = True
